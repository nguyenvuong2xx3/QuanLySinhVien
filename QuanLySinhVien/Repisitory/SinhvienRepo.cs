...
namespace Repository
{
    public class SinhvienRepo : ISinhvienRepo
    {
        ...
        private static SinhvienRepo instante = null;

        public static SinhvienRepo Instance
        {
            get
            {
                if (instante == null)
                {
                    return new SinhvienRepo();
                }
                return instante;
            }
        }
        ...
        public bool DeleteSinhvien(string id)
        {
            ...
            try
            {
                List<Diemsv> diemList = DiemsvRepo.Instance.GetDiemsvByMasv(id);
                foreach (var diem in diemList)
                {
                    _db.Diemsvs.Remove(diem);
                }
                List<Account> accounts = AccountRepo.Instance.GetAccountByMasv(id);
